...
public class MyRecieveFormAction implements CommandAction{

	@Override
	public String requestPro(HttpServletRequest request,
			HttpServletResponse response) throws Throwable {
		System.out.println("MyRecieveFormAction 들어옴");
		//세션
		...
		count=dao.getRecieveReview(loginMember.getId());//판매 내역 글 갯수
		
		
		if(count>0){ //글 존재
			rlist=dao.getRecievelist(loginMember.getId() ,startRow, pageSize);
		}else{
		...
		return "/review/myrecieveForm.jsp";
	}
