                    ...
                    <ul class="list-group list-group-flush border-bottom">
                    	<c:forEach var="msg" items="${list}">
	                        <li class="list-group-item pb-1 mb-1 px-0">
	                            ...
	                            <div class="mt-1" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
	                                <a class="text-decoration-none <c:if test="${msg.status == 0}">text-body</c:if> <c:if test="${msg.status == 1}">text-muted</c:if>" href="${ctxpath}/msg/contentForm.do?id=${msg.id}"><b>${msg.receiver_nickname}님에게 보낸 메세지</b></a>
	                            </div>
	                        </li><!-- 알림 반복문 -->
                        </c:forEach>
                    </ul><!-- 알림 목록 -->
